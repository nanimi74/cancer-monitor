  ...
  static const privacy = LegalScreen(
    ...
    sections: [
      ...
      LegalSection(
        ...
        body:
            'AI 분석은 사용자가 AI분석 화면에서 분석을 요청하고 데이터 전송에 동의할 때만 실행됩니다. 앱은 외부 AI 처리자를 직접 호출하지 않고 Firebase Cloud Functions 서버를 경유해 Anthropic Claude API에 분석을 요청합니다.\n\nAI 분석 요청에는 선택한 회차와 이전 회차의 증상 기록, 식사량과 식사 메모, 음수량, 걸음수, 배변 여부와 상태, 컨디션 변화, 주요 컨디션 메모, 체중 기록, 사용자 정보 중 나이, 성별, 암종, 단계, 진단일, 전이 여부, 치료 방식, 치료 시작일, 키, 기타정보가 포함될 수 있습니다. 이름, 이메일, Firebase UID, 로그인 제공자 정보는 AI 분석 요청에 포함하지 않습니다.\n\nAnthropic Claude API로 전송되는 정보는 기록 요약과 흐름 정리를 위해 사용되며, 당사는 외부 처리자가 동등하거나 그에 준하는 보호 기준에 따라 데이터를 처리하도록 관리합니다.\n\nAI 분석 결과는 참고용 정보이며 의학적 진단, 처방, 치료 변경, 응급 판단을 대체하지 않습니다. 건강과 관련된 중요한 결정은 담당 의료진 또는 전문가와 상의해야 합니다.',
      ),
      LegalSection(
