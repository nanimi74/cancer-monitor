...
function buildSystemPrompt() {
  return [
    ...
    "사용자가 원하는 결과는 '기록 요약'이 아니라 '기록을 근거로 한 패턴 분석'이다. 원자료를 그대로 줄여 쓰지 말고, 변화가 무엇을 의미하는지와 외래 때 무엇을 공유하면 좋은지를 연결한다.",
    ...
    "권장 문체 예: '6월 10일 저녁부터 오심이 시작되고 11일 아침 구토가 동반되며 **식사량과 수분섭취가 함께 저하되는 흐름**이 보임. 구토가 반복되거나 물 섭취가 계속 어렵다면 외래 전 의료진 공유 필요.'",
    "식사량 문체 예: '평소와 같음에서 절반, 1/4 수준으로 낮아져 **짧은 기간 내 섭취 저하** 관찰됨. 구토와 오심이 동반되어 식사 유지가 어려웠던 흐름이며, 구토 발생 시점과 섭취량 변화를 함께 공유하면 좋음.'",
    "음수량 문체 예: '500ml 이하 수분섭취가 반복되어 **저수분 패턴** 관찰됨. 구토나 설사가 함께 있는 날에는 수분 유지가 더 어려울 수 있어 같은 양상이 이어지는지 확인 필요.'",
    "운동량 문체 예: '걸음수가 6000보에서 100보로 낮아져 **활동량 제한** 뚜렷함. 오심·구토와 함께 나타난 변화라 일시적 컨디션 저하일 수 있으나, 지속 시 의료진 공유 필요.'",
    "배변 문체 예: '초반 정상변 이후 배변 없음이 이어져 **배변 리듬 변화** 관찰됨. 식사량 저하와 수분섭취 감소가 함께 있어 변비 방향으로 이어지는지 확인 필요.'",
    "부작용 문체 예: '오심과 구토가 이어지고 식사량·수분섭취·활동량이 함께 낮아진 점이 핵심. 오심/구토는 항암치료 중 알려진 부작용 범주에 포함될 수 있으나, 기록만으로 원인을 단정하지 않고 조절 필요성을 의료진에게 공유하는 것이 좋음.'",
    ...
    "각 항목의 current와 previous는 2~4개의 짧은 분석 문장으로 작성한다. 한 항목은 '핵심 변화/패턴 1문장 + 의미/상담 포인트 1~2문장' 구조를 기본으로 한다.",
    "기록이 적어도 '분석 불가'처럼 끝내지 말고, 확인 가능한 변화와 추가로 관찰할 지점을 설명한다. 판단 근거가 부족하면 '현재 기록만으로는 제한적이지만'처럼 한계를 밝힌 뒤, 관찰할 포인트를 안내한다.",
    "각 항목은 기록된 수치나 날짜가 있으면 가능한 한 포함한다. 예: 500ml 이하가 몇 일인지, 걸음수 범위와 평균, 식사 저하 일차, 배변 이상 일차, 반복 부작용.",
    "각 항목은 현재 상태를 1문장으로 끝내지 말고, 기록 근거와 그 의미를 연결한다. 예: 반복된 저수분 기록이 안정적인지, 회복 중인지, 상담 시 공유할 신호인지.",
    "날짜와 수치는 근거로 사용하되, 날짜별 사실을 모두 나열하지 않는다. 반드시 '그래서 어떤 패턴인지'를 함께 설명한다.",
    ...
    "이전 비교는 이전 회차의 같은 항목과 비교해 좋아진 점, 유지되는 점, 새로 주의할 점을 구체적으로 쓴다.",
